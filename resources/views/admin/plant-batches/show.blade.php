...
<div class="container">
    ...
    <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
            <div>
                <h3 style="margin:0 0 4px;font-size:1.3rem;">{{ $plantBatch->product?->product_name ?? '-' }}</h3>
                <p style="margin:0;color:#888;font-size:.88rem;">
                    บันทึกโดย {{ $plantBatch->creator?->fullname ?? '-' }}
                    · {{ $plantBatch->created_at?->format('d/m/Y H:i') }}
                </p>
            </div>
            <span class="status-badge s-{{ $plantBatch->status }}">{{ $plantBatch->status_label }}</span>
        </div>
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                <label>📦 จำนวนที่เก็บได้จริง</label>
                <strong>{{ $plantBatch->qty_harvested ? number_format($plantBatch->qty_harvested,1).' กระถาง' : '—' }}</strong>
            </div>
            @if($plantBatch->actual_harvest_date)
            <div class="info-item">
        ...
        <hr>
        <div class="info-item">
            <label>📊 StockLog (ตอนเก็บเกี่ยว)</label>
            <strong>
                +{{ number_format($plantBatch->stockLog->qty_change,1) }} กระถาง
                → สต็อกหลังเพิ่ม: {{ number_format($plantBatch->stockLog->qty_after,1) }}
            </strong>
        </div>
        @endif
    </div>
    ...
    <div class="card action-card">
        <h3>🌾 เก็บเกี่ยวและเพิ่มสต็อก</h3>
        <p style="margin:-8px 0 18px;color:#888;font-size:.85rem;">จำนวนจะถูกเพิ่มเข้าสต็อกสินค้าทันทีพร้อมบันทึก StockLog และส่งแจ้งเตือน LINE</p>
        <form method="POST" action="{{ route('admin.plant-batches.harvest', $plantBatch) }}"
              onsubmit="return confirm('ยืนยันการเก็บเกี่ยว?\nสต็อกจะเพิ่มขึ้นทันที')">
            @csrf
            <div class="form-row">
                <div class="form-group">
                    <label>จำนวนที่เก็บได้จริง (กระถาง) *</label>
            ...
            <div class="form-group">
                <label>หมายเหตุการเก็บเกี่ยว</label>
                <textarea name="harvest_notes"
                    placeholder="เช่น คุณภาพดีมาก, มี 5 กระถางที่ยังเล็กเกินไป...">{{ old('harvest_notes') }}</textarea>
            </div>
            <button type="submit" class="btn btn-green">🌾 ยืนยันเก็บเกี่ยว — เพิ่มเข้าสต็อก</button>
        </form>
    </div>
    ...
    <div class="card action-card danger">
        ...
        <form method="POST" action="{{ route('admin.plant-batches.fail', $plantBatch) }}"
            ...
            <div class="form-group">
                <label>สาเหตุที่ล้มเหลว</label>
                <textarea name="fail_notes" placeholder="เช่น เชื้อราระบาด, น้ำท่วม, แมลงกิน...">{{ old('fail_notes') }}</textarea>
            </div>
            <button type="submit" class="btn btn-red">❌ บันทึกว่าล้มเหลว</button>
        </form>
    </div>
